...
if (isset($_SESSION['user_id']) && isset($_SESSION['user_username']) && isset($_SESSION['account_type'])) {
    ...
    $stmt = $conn->prepare("UPDATE `appointment_requests` SET `appoint_status_id` = ?, `verdict_by` = ?, `cancel_reason_id` = ?, `cancel_reason_other` = ?, `cancel_datetime` = ? WHERE `id` = ?");
    $stmt->bind_param("iiissi", $appoint_status_id, $patient_id, $cancel_reason_id, $cancel_reason_other, $cancel_datetime, $appoint_id);
