...
function insertInfo($user_id, $lname, $fname, $mname, $contnumber, $bdate, $age, $gender, $religion, $nationality, $occupation, $address) {
    ...
    $stmt = $conn->prepare("INSERT INTO `secretary_info`(`accounts_id`, `lname`, `fname`, `mname`, `contactno`, `bdate`, `age`, `gender`, `religion`, `nationality`, `occupation`, `address`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)");
...
function updateInfo($user_id, $lname, $fname, $mname, $contnumber, $bdate, $age, $gender, $religion, $nationality, $occupation, $address) {
    ...
    $stmt = $conn->prepare("UPDATE `secretary_info` SET `lname` = ?, `fname` = ?, `mname` = ?, `contactno` = ?, `bdate` = ?, `age` = ?, `gender` = ?, `religion` = ?, `nationality` = ?, `occupation` = ?, `address` = ? WHERE `accounts_id` = ?");
