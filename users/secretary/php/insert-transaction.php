...
function updateAppointmentStatus($conn, $appointment_requests_id, $appoint_status_id, $timestamp) {
    if ($appoint_status_id == 5) {
        $stmt = $conn->prepare("UPDATE appointment_requests SET appoint_status_id = ?, completed_datetime = ? WHERE id = ?;");
        $stmt->bind_param("isi", $appoint_status_id, $timestamp, $appointment_requests_id);        
    } else {
...
if (isset($_SESSION['user_id']) && isset($_SESSION['user_username']) && isset($_SESSION['account_type'])) {
    ...
    if (count($procedures_id) === count($amount_paid) && 
        ...
        count($procedures_id) === count($appointment_requests_id)) {
        ...
        try {
            for ($i = 0; $i < count($procedures_id); $i++) {
                ...
                insertTransaction($conn, $patient_id, $secId, $payment_id, $payment_ref_no, $amount_paid[$i], 
                    $balance, $timestamp, $appointment_requests_id[$i], $procedures_id[$i]
                );

                updateTreatmentHistory($conn, $patient_id, $balance, 
                $appointment_requests_id[$i], $procedures_id[$i]
                );

                $appoint_status_id = checkBalances($conn, $appointment_requests_id[$i]);
                updateAppointmentStatus($conn, $appointment_requests_id[$i], $appoint_status_id, $timestamp);
            };
